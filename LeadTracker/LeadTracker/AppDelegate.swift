...
//  LeadTracker
//
//  Created by Barak Shushan on 9/13/16.
...
class AppDelegate: UIResponder, UIApplicationDelegate
{
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "com.LeadTracker" in the application's documents Application Support directory.
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("LeadTracker", withExtension: "momd")!
    ...
    func parseCSV (contentsOfURL: NSURL, encoding: NSStringEncoding, error: NSErrorPointer) -> [(user_name:String, school_name:String, date:String)]?
    {
        // Load the CSV file and parse it
        let delimiter = "|"
        var leads:[(user_name:String, school_name:String, date:String)]?
       ...
       // let content = String(contentsOfURL: contentsOfURL, encoding: encoding, error: error)
        ...
        if let data = NSData(contentsOfURL: contentsOfURL)
        {
                ...
                for line in lines
            {
                ...
                if(values.count > 5)
                {
                    //print("user name \(values[4])")
                    //print("school name \(values[2])")
                    //print("user name \(values[1])")

                    let item = (user_name: values[4] + " " + values[5], school_name: values[2], date: values[1])
                    leads?.append(item)
                ...
                for lead in leads
                    {
                        ...
                        //print(lead.user_name)
                        leadItem.user_name = lead.user_name
                        leadItem.school_name = lead.school_name
                        leadItem.date = lead.date
                        do
                        {
                            try managedObjectContext.save()
                        }
